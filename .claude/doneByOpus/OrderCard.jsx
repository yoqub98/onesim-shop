...
const OrderCard = ({ order, onActivate, onViewDetails, onTopup }) => {
  ...
  useEffect(() => {
    const fetchLiveData = async () => {
      ...
      try {
        ...
        console.log('✅ [OrderCard LIVE] Full response structure:', JSON.stringify(response, null, 2));

        // Handle multiple possible response structures from the backend proxy:
        // Structure 1: { success, obj: { esimList: [...] } }  (direct passthrough from eSIMAccess)
        // Structure 2: { success, data: { esimList: [...] } }  (backend wraps in data)
        // Structure 3: { success, data: { obj: { esimList: [...] } } }  (double wrapped)
        // Structure 4: { success, esimList: [...] }  (flat structure)
        let esimList = null;
        ...
        if (response?.obj?.esimList?.length > 0) {
          ...
          console.log('📊 [OrderCard LIVE] Found esimList at response.esimList');
        }
        ...
        if (esimList && esimList.length > 0) {
          ...
          console.log('📊 [OrderCard LIVE] Live eSIM data:', {
            ...
            packageCount: esimData.packageList?.length || 0,
          });
          ...
          setLiveData({
            ...
            supportTopUpType: esimData.supportTopUpType,
          });
        } else {
          console.warn('⚠️ [OrderCard LIVE] No eSIM data found in response. Keys:', Object.keys(response || {}));
          if (response?.data) {
            console.warn('⚠️ [OrderCard LIVE] response.data keys:', Object.keys(response.data || {}));
          }
        }
      } catch (err) {
  ...
  // Check if top-up is supported - use LIVE data when available
  const supportsTopup = canTopup({
    ...order,
    // Override with live data if available
    ...(liveData ? {
      esim_status: liveData.esimStatus || order.esim_status,
      smdp_status: liveData.smdpStatus || order.smdp_status,
    } : {}),
  });

  // Check if this package supports top-up (based on supportTopUpType)
  // supportTopUpType: 1 or 2 means reloadable
  // Use live data value if available
  const liveSupportTopUpType = liveData?.supportTopUpType;
  const effectiveTopUpType = liveSupportTopUpType !== undefined ? liveSupportTopUpType : order.support_topup_type;
  const packageSupportsTopup = effectiveTopUpType === 1 || effectiveTopUpType === 2 || !effectiveTopUpType;
