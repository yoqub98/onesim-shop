...
export const queryEsimProfile = async (orderNo) => {
  try {
    ...
    console.log('🔍 [SERVICE] Profile response raw:', JSON.stringify(data, null, 2).substring(0, 500));

    // Find esimList regardless of response structure
    const esimList = data?.obj?.esimList 
      || data?.data?.esimList 
      || data?.data?.obj?.esimList 
      || data?.esimList 
      || [];

    console.log('🔍 [SERVICE] Profile response parsed:', {
      success: data.success,
      hasObj: !!data.obj,
      hasData: !!data.data,
      hasEsimList: esimList.length > 0,
      esimCount: esimList.length,
      responseKeys: Object.keys(data || {}),
      firstEsimStatus: esimList[0]?.esimStatus || 'N/A',
    });

    if (!response.ok) {
      throw new Error(data.error || `HTTP ${response.status}: Failed to query eSIM profile`);
    }

    // Consider success if we found esimList data, even if success flag is missing
    if (!data.success && esimList.length === 0) {
      throw new Error(data.error || data.errorMsg || 'Failed to query eSIM profile');
    }

    // Normalize response: ensure obj.esimList exists for consistent consumption
    if (!data.obj?.esimList && esimList.length > 0) {
      console.log('🔍 [SERVICE] Normalizing response - adding obj.esimList');
      if (!data.obj) data.obj = {};
      data.obj.esimList = esimList;
    }
